...
let parseBoards (boards : int[][]) : BingoBoard list =
    boards |> Seq.chunkBySize 5 |> Seq.map convertChunkToBoard |> Seq.toList
...
let main argv =
    ...
    let boards: int[][] = readAllLines reader
    let mutable boards: BingoBoard list = parseBoards boards

    let losingScore = numbers |> Seq.tryPick (
        fun num ->
            let winners = boards |> List.filter (
                fun b ->
                    b.playValue(num)
                    b.isWinner()
            )
            boards <- boards |> List.except (
                if boards.Length <> 1 then winners
                else List.empty
            )

            if boards.Length = 1 && boards.Head.isWinner() then option.Some(boards.Head.calculateScore(num))
            else option.None
    )

    match losingScore with
    | Some score -> printfn $"%d{score}"
    | None -> printfn "No losing board found"
