...
walk <- function(nodes, connections, node, seen, visitedSmallTwice) {
  # Reached the end
  if (node@name == "end") {
    return(1)
  }
  ...
  if (node@name %in% seen) {
    if (node@name != "start" && !visitedSmallTwice) {
      # Give visiting a small cave twice a try
  ...
  if (!node@isBig && !(node@name %in% seen)) {
  ...
  for (c in con) {
    newNode <- nodes[[c]]
    newCount <- walk(nodes, connections, newNode, seen, visitedSmallTwice)
    if (newCount == -1) {
      next
    }
...
count <- walk(nodes, connections, nodes[["start"]], list(), FALSE)
print(count)
